...
class CnvNet(nn.Module):
    ...
    def loadNetwork(cls):
        cnvnet = CnvNet()
        cnvnet.load_state_dict(torch.load(PATH))
        return cnvnet
    ...
    def trainNetwork(self, iterations):
        for epoch in range(iterations):  # loop over the dataset multiple times
            ...
            running_loss = 0.0
            for i, data in enumerate(self.imgData.trainloader, 0):
    ...
    def testNetworkSample(self):
        dataiter = iter(self.imgData.testloader)
        images, labels = dataiter.next()

        # print images
        self.imgData.imshow(torchvision.utils.make_grid(images))
...
def trainAndReport(iterations):
    cnvnet = CnvNet()
    cnvnet.trainNetwork(iterations)
    cnvnet.saveNetwork()
    cnvnet.testNetworkSet()
    cnvnet.classAccuracy()
